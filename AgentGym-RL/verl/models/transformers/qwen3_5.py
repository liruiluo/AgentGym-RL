...
"""Packed-sequence and fused PPO forwards for Qwen3.5.

This is a focused backport of verl commits 3ba8181 and 6a6242f.  AgentGym-RL
uses the text-only Qwen3.5 model for actor and critic training.  The packed GDN
path follows upstream Qwen3.5 support, while the fused PPO head adapts upstream's
``Qwen3_5ForConditionalGeneration`` implementation to the actual
``Qwen3_5ForCausalLM`` actor loaded here.
"""
...
class Qwen3_5PPOOutput:
    """Minimal actor output that avoids materializing full-vocabulary logits."""
...
def qwen3_5_causal_lm_forward_dispatch(self, *args, **kwargs):
    """Route only explicitly marked PPO calls through the fused text head."""

    use_fused_ppo = bool(kwargs.pop("_verl_fused_ppo", False))
    if use_fused_ppo:
        return qwen3_5_text_fused_ppo_forward(self, *args, **kwargs)
    original_forward = getattr(type(self), _ORIGINAL_CAUSAL_LM_FORWARD_ATTR)
    return original_forward(self, *args, **kwargs)


def qwen3_5_text_fused_ppo_forward(
    self,
    ...
    **kwargs,
    ...
    """Compute PPO log-probability and entropy without a ``[T, V]`` tensor.

    This mirrors current VERL's torch fused Qwen3.5 head.  ``shift_labels`` is
    supplied by the packed actor from the full sequence so labels never roll
    across packed-example boundaries or a future sequence-parallel shard.
    """

    from verl.utils.experimental.torch_functional import FusedLinearForPPO

    if input_ids is None:
        raise ValueError("Qwen3.5 fused PPO forward requires input_ids.")
    if not isinstance(temperature, (int, float)):
        raise TypeError("Qwen3.5 fused PPO forward requires one scalar temperature.")
    ...
    if rolled_labels.shape != input_ids.shape:
        raise ValueError(
            "Qwen3.5 fused PPO labels must match packed input shape: "
            f"labels={tuple(rolled_labels.shape)} input={tuple(input_ids.shape)}."
        )
    ...
    log_probs, entropy = FusedLinearForPPO()(  # exact upstream torch backend
        hidden_states=hidden_states,
        vocab_weights=vocab_weights,
        input_ids=rolled_labels,
        temperature=float(temperature),
    )
    return Qwen3_5PPOOutput(log_probs=log_probs, entropy=entropy)
