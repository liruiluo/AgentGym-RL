...
_AGENTMEMORY_FILESYSTEM_MEMORY_GUIDANCE = (
    ...
    "purchase, the first action must use shell_command to read the canonical continuation "
    "checkpoint exactly with `cat .agent_memory/CONTINUATION.md`; do not search, click, or "
    "write a replacement fact until the preceding session's saved evidence has appeared in "
    "shell output. If that exact read fails or returns empty output, retry the exact `cat` "
    "command once; never loop on an unchanged successful read. An empty result is never "
    ...
    "only from that exact saved field and value; "
