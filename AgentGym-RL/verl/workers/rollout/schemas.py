...
_AGENTMEMORY_FILESYSTEM_MEMORY_GUIDANCE = (
    ...
    "value needed later; do not replace it with a generic certified, natural, normal, or "
    "boolean label. Keep an Add File note to one content line beginning with `+Confirmed`; "
    "every Add File content line must begin with `+`. Shopping-session transitions use "
    "latest-observation-only input, so your earlier action reply and the filename you chose "
    "are not shown after advancing. In a session whose choice depends on the preceding "
    "purchase, the first action must use shell_command to discover and print saved Confirmed "
    ...
    "printed, follow this order without skipping steps: choose the branch only from that line; "
    "copy the chosen approved card's complete Product title into search[...] without shortening "
    "it; open only a result whose complete visible title exactly equals the card, including size, "
    "count, and pack qualifiers; only after that exact product page is open, copy the card's "
    "entire `Confirmed <field>: <value>` line verbatim into one new Add File note; then, after "
    "Done!, use click[Buy Now]. Never invent a `Confirmed ... to buy:` field and never put the "
    "product title in place of the card's Confirmed field and value. After an Add "
    "File action returns Done!, keep that successful note "
    "unchanged: do not add it again, delete it, or rewrite it. Do not write on every turn "
    "and do not write when no later decision needs the fact. For a new note, use Add File "
    "with a new path; never use Update File for a missing path or repeat Add File for an "
    "existing path. The environment supplies tool feedback on the "
...
_AGENTMEMORY_RECENCY_FILESYSTEM_MEMORY_GUIDANCE = (
    ...
    "text to the action. To create the record, a complete reply starts with apply_patch, uses "
    "*** Begin Patch and *** Add File with a new path, copies the task's exact record as its "
    "single + line, and ends with *** End Patch. To replace it, a later complete reply uses "
    "*** Update File on that same path, @@, one - line exactly matching the old record, and "
    "one + line exactly matching the new task-provided record. Run shell_command in a separate "
    "turn after tool feedback when the record must be read. No concrete preference value or "
    "filename is demonstrated here: choose both only from policy-visible task facts. Keep each "
