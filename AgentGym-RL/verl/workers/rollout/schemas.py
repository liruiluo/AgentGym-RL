...
_AGENTMEMORY_FILESYSTEM_MEMORY_GUIDANCE = (
    ...
    "boolean label. After advancing, read that same note's contents before the dependent "
    "decision. Listing a directory or reading a different file is not reading the note. Do "
    "not write on every turn and do not write when no later decision needs the fact. For a "
    "new note, use Add File with a new path; never use Update File for a missing path or "
    "repeat Add File for an existing path. The environment supplies tool feedback on the "
