...
_AGENTMEMORY_FILESYSTEM_MEMORY_GUIDANCE = (
    ...
    "boolean label. After advancing into a session whose choice depends on the preceding "
    "purchase, do not search, click, or write a replacement fact until you have used "
    "shell_command to read the preceding session's note contents. If you forgot its path, "
    "locate the candidate note and then read that file; never infer or recreate the missing "
    "value from the choice table. Listing a directory or reading a different file is not "
    "reading the note. After an Add File action returns Done!, keep that successful note "
