...
from agentmemorygym_verl.heldout_eval_contract import (
    ...
    BATCH_SCHEMA,
    CANONICAL_ROUTES,
    FINAL_EVALUATION_CONTRACT_SCHEMA,
...
from agentmemorygym_verl.heldout_eval import (
    ...
    _schedule_counts_and_panel_authority,
    derive_eval_config,
    load_eval_plan,
    run_contract,
    run_evaluation,
...
TEST_ROUTE_COUNTS = {
    route_id: 128 for route_id in CANONICAL_ROUTES
}
...
def _make_spec(
    ...
    *,
    ...
    if len(set(route_counts.values())) != 1:
        raise ValueError("final-panel fixture requires equal per-route counts")
    target_per_environment = counts[0]
    route_by_id = {route["route_id"]: route for route in routes}
    final_panel = root / "final-panel-manifest.json"
    final_panel_sha256 = _write_json(
        final_panel,
        {
            "schema": FINAL_PANEL_SCHEMA,
            "status": "PASS",
            "evaluation_label": "CAMG_FINAL_HELDOUT_TEST_FIXTURE",
            "paper_final_evaluation_set": True,
            "same_rows_for_all_methods": True,
            "selection_uses_model_outputs_or_rewards": False,
            "active_training_inputs_modified": False,
            "training_input_hashes_before_after_equal": True,
            "aggregate_metric": FINAL_PANEL_AGGREGATE,
            "target_per_environment": target_per_environment,
            "total_denominator": sum(counts),
            "environments": {
                FINAL_PANEL_ROUTE_NAMES[route_id]: {
                    "count": route_counts[route_id],
                    "source_count": route_counts[route_id] + 10,
                    "routing_path": str(
                        (root / route_by_id[route_id]["schedule"]).resolve()
                    ),
                    "routing_sha256": route_by_id[route_id]["schedule_sha256"],
                    "selection_uses_model_outputs_or_rewards": False,
                }
    ...
    final_evaluation_contract = root / "final-evaluation-contract.json"
    final_evaluation_contract_sha256 = _write_json(
        final_evaluation_contract,
        {
            "schema": FINAL_EVALUATION_CONTRACT_SCHEMA,
            "status": "FROZEN_PENDING_TERMINAL_CHECKPOINT_EVALUATION",
            "terminal_checkpoint_only": True,
            "intermediate_checkpoint_results_allowed_in_paper_table": False,
            "same_task_rows_for_every_method": True,
            "per_environment_denominator": target_per_environment,
            "total_task_count": sum(counts),
            "primary_aggregate": FINAL_PANEL_AGGREGATE,
            "routing_files": {
                FINAL_PANEL_ROUTE_NAMES[route_id]: str(
                    (root / route_by_id[route_id]["schedule"]).resolve()
                )
                for route_id in CANONICAL_ROUTES
            },
    ...
    spec = {
        "schema": SPEC_SCHEMA,
        "agent_name": AGENT_NAME,
        "panel_id": "camg-final-heldout-test-fixture",
        "route_registry_sha256": route_registry_sha256,
        "final_panel_manifest": final_panel.name,
        "final_panel_manifest_sha256": final_panel_sha256,
        "final_evaluation_contract": final_evaluation_contract.name,
        "final_evaluation_contract_sha256": final_evaluation_contract_sha256,
        "routes": routes,
    }
...
def _compose(root: Path, counts=(2, 2, 2, 2)):
...
def _compaction_row(
    ...
    *,
    ...
    summary = "retain the exact constraints and verified progress"
    encoded = summary.encode("utf-8")
    row["action"] = summary
    row["response_token_count"] = 9
    row["env_info_after"] = {
        "episode_source_identity": _episode_source_identity(input_row)
    }
    row["context_transition"] = {
        "schema": "agentmemory_task_neutral_context_transition_v1",
        "operation": "replace_messages",
    }
    row["action_submission"] = {
        "raw_policy_output": summary,
        "parser_status": "compactionrl_summary_not_dispatched",
    }
    row["wrapper_evidence"] = {
        ...
        "summary_byte_count": len(encoded),
...
class TestHeldoutSchedule(unittest.TestCase):
    def test_compose_is_deterministic_and_preserves_local_index(self):
        with tempfile.TemporaryDirectory() as directory:
            ...
            self.assertEqual(
                [row["data_idx"] for row in rows],
                [100, 0, 100, 100, 101, 1, 101, 101],
            )
    ...
    def test_rejects_source_hash_drift_and_reserved_eval_identity(self):
        with tempfile.TemporaryDirectory() as directory:
            ...
            with self.assertRaisesRegex(ValueError, "routing sha256 mismatch"):
                compose_heldout_schedule(
                    spec_path,
                    expected_spec_sha256=spec_sha256,
            ...
            panel_path = root / spec["final_panel_manifest"]
            panel = json.loads(panel_path.read_text())
            panel["environments"]["shop"]["routing_sha256"] = source_sha
            spec["final_panel_manifest_sha256"] = _write_json(panel_path, panel)
    ...
    def test_rejects_final_evaluation_contract_drift(self):
        with tempfile.TemporaryDirectory() as directory:
            root = Path(directory)
            ...
            contract["same_task_rows_for_every_method"] = False
            spec["final_evaluation_contract_sha256"] = _write_json(
                contract_path, contract
            )
            spec_sha256 = _write_json(spec_path, spec)
            with self.assertRaisesRegex(ValueError, "evaluation contract drifted"):
                compose_heldout_schedule(
                    spec_path,
                    expected_spec_sha256=spec_sha256,
                    output_path=root / "out.jsonl",
    ...
    def test_rejects_route_count_not_authorized_by_final_panel(self):
        with tempfile.TemporaryDirectory() as directory:
            ...
            panel_path = root / spec["final_panel_manifest"]
            panel = json.loads(panel_path.read_text())
            panel["environments"]["shop"]["count"] += 1
            spec["final_panel_manifest_sha256"] = _write_json(panel_path, panel)
            spec_sha256 = _write_json(spec_path, spec)
    ...
    def test_formal_runner_requires_exact_four_by_128_panel(self):
        with tempfile.TemporaryDirectory() as directory:
            _, _, _, manifest_path = _compose(
                Path(directory), counts=(1, 1, 1, 1)
            )
            metadata = json.loads(manifest_path.read_text())
            with self.assertRaisesRegex(ValueError, "exactly 128 rows per route"):
                _schedule_counts_and_panel_authority(metadata)
...
class TestHeldoutMaterialization(unittest.TestCase):
    ...
    def test_compaction_evidence_is_mandatory_and_native_dispatch_fails_closed(self):
        with tempfile.TemporaryDirectory() as directory:
            rows, _, _, _ = _compose(Path(directory), counts=(1, 1, 1, 1))
            source = rows[1]
            missing = _action_row(source, order=0, length=1, success=True)
            missing["context_transition"]["operation"] = "replace_messages"
            missing["wrapper_evidence"] = {}
            with self.assertRaisesRegex(ValueError, "CompactionRL evidence failed"):
                _materialize_records(source, [missing])

            dispatched = _compaction_row(
                source, order=0, length=1, success=True
            )
            dispatched["wrapper_evidence"]["native_environment_call_count"] = 1
            with self.assertRaisesRegex(ValueError, "native environment call"):
                _materialize_records(source, [dispatched])
    ...
    def _plan_fixture(self, root: Path):
        ...
        model_manifest_hash = _write_json(
            model_manifest,
            {
                ...
                "checkpoint_step": 200,
                "training_run_id": "compactionrl-formal200",
...
""",
        ...
        kwargs = {
            "run_id": "compactionrl-heldout-eval-v1",
            "run_dir": root / "run",
    ...
    def test_plan_verifies_all_inputs_and_derives_greedy_standalone_config(self):
        with tempfile.TemporaryDirectory() as directory:
            ...
            self.assertEqual(
                contract["schedule"]["final_panel_authority"]["route_counts"],
                TEST_ROUTE_COUNTS,
            )
    ...
    def test_commit_interval_and_receipt_metric_tampering_fail_closed(self):
        with tempfile.TemporaryDirectory() as directory:
            ...
            receipt["compactionrl_evidence"]["real"]["totals"][
                "valid_compactions"
            ] = 999
            _write_json(receipt_path, receipt)
            with self.assertRaisesRegex(ValueError, "CompactionRL evidence differs"):
                verify_batch_directory(run_dir / "batches/batch-000000")
    ...
    def test_runtime_uses_standalone_server_and_agent_loop_apis_then_resumes(self):
        with tempfile.TemporaryDirectory() as directory:
            ...
            self.assertEqual(calls["generate"], 8)
