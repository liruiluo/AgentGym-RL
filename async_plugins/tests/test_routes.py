...
class RouteRegistryTests(unittest.TestCase):
    ...
    def test_file_registry_accepts_canonical_route_subset(self) -> None:
        with tempfile.TemporaryDirectory() as directory:
            payload = _registry_payload()
            payload["routes"] = [payload["routes"][1]]
            path, digest = self._write(directory, payload)
            registry = load_route_registry(path, expected_sha256=digest)
            self.assertEqual(registry.route_ids, ("swesmith",))

            registry_with_expected = load_route_registry(
                path,
                expected_sha256=digest,
                expected_route_ids=("swesmith",),
            )
            self.assertEqual(registry_with_expected.route_ids, ("swesmith",))

    def test_file_registry_rejects_noncanonical_subset_order_and_expected_mismatch(self) -> None:
        with tempfile.TemporaryDirectory() as directory:
            payload = _registry_payload()
            payload["routes"] = [payload["routes"][1], payload["routes"][0]]
            path, digest = self._write(directory, payload)
            with self.assertRaisesRegex(ValueError, "canonical ordered route subset"):
                load_route_registry(path, expected_sha256=digest)

            payload = _registry_payload()
            payload["routes"] = [payload["routes"][1]]
            path, digest = self._write(directory, payload)
            with self.assertRaisesRegex(ValueError, "expected route order mismatch"):
                load_route_registry(
                    path,
                    expected_sha256=digest,
                    expected_route_ids=("webshop",),
                )
