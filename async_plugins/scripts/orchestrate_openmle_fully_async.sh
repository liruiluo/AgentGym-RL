...
Usage: orchestrate_openmle_fully_async.sh \
  ...
  [--critic-ppo-infer-max-tokens-per-gpu N]
EOF
  exit 64
}
...
while (($#)); do
  case "$1" in
    ...
    --critic-ppo-infer-max-tokens-per-gpu) CRITIC_PPO_INFER_MAX_TOKENS_PER_GPU=${2:?}; shift 2 ;;
    *) usage ;;
  esac
done
...
LAUNCH_TUNING_ARGS=(
  ...
  --critic-ppo-infer-max-tokens-per-gpu "$CRITIC_PPO_INFER_MAX_TOKENS_PER_GPU"
)
