...
class _Audit:
    ...
    def audit_config(self) -> None:
        ...
        try:
            ...
            hydra = _load_yaml(hydra_path, "Hydra config")
        except Exception as exc:
    ...
    def audit_file_logger(self) -> None:
        ...
        for index, row in enumerate(rows):
            self.check(
                ...
                f"FileLogger row {index} has no integer step",
            )
            if isinstance(row.get("step"), int) and not isinstance(
                row.get("step"), bool
            ):
                observed_steps.append(int(row["step"]))
            data = row.get("data")
            ...
            for key, value in data.items():
                ...
                if "actor" in folded and _finite_positive(value):
                    actor_nonzero = True
                if "critic" in folded and _finite_positive(value):
                    critic_nonzero = True
            if "rollout_corr/bypass_real_token_count" in data:
                bypass_evidence_rows += 1
                self.check(
                    _finite_positive(data["rollout_corr/bypass_real_token_count"]),
                    f"FileLogger row {index} has no compared real-token count",
                )
                if _finite_positive(data["rollout_corr/bypass_real_token_count"]):
                    bypass_real_tokens += int(
                        data["rollout_corr/bypass_real_token_count"]
                    )
                self.check(
                    data.get("rollout_corr/bypass_max_abs_diff") in (0, 0.0),
                    f"FileLogger row {index} reports an old/rollout logprob mismatch",
                )
            if data.get("parameter_update_probe/actor/changed") in (True, 1, 1.0):
                actor_probe_rows += 1
            if data.get("parameter_update_probe/critic/changed") in (True, 1, 1.0):
                critic_probe_rows += 1
        self.check(actor_nonzero, "FileLogger has no nonzero actor update metric")
        self.check(critic_nonzero, "FileLogger has no nonzero critic update metric")
        self.check(
            validation_metrics == 0,
        ...
        if self.expected is not None:
            ...
            self.check(
                observed_steps == list(range(1, publications + 1)),
                "FileLogger publication steps are incomplete or out of order",
            )
    ...
    @staticmethod
    def _policy_row(record: Mapping[str, Any], document: Mapping[str, Any]) -> bool:
    ...
    @staticmethod
    def _successful_execution(record: Mapping[str, Any]) -> Mapping[str, Any] | None:
        ...
        if not isinstance(execution, Mapping):
            return None
        if (
            info.get("action_status") != "completed"
            or execution.get("status") != "completed"
            or execution.get("exit_code") != 0
        ):
            return None
        attempts = _at(info, "counter_delta.execution_attempt_count", 0)
        execution_delta = execution.get("execution_attempt_delta", 0)
        if not isinstance(attempts, int) or attempts <= 0:
            return None
        if not isinstance(execution_delta, int) or execution_delta <= 0:
            return None
        return execution
    ...
    def _has_memory_chain(
        ...
        policy = [cls._policy_row(record, document) for record, document in episode]
        for write_index, (write, _document) in enumerate(episode):
            if not policy[write_index]:
                continue
            write_execution = cls._successful_execution(write)
            if write_execution is None:
                continue
            changed = write_execution.get("changed_paths")
            if not isinstance(changed, Sequence) or isinstance(changed, (str, bytes)):
                continue
            for raw_path in changed:
                external_path = str(raw_path)
                if not external_path or external_path.startswith(("/", "..")):
                    continue
                for compact_index in range(write_index + 1, len(episode)):
                    compact = episode[compact_index][0]
                    evidence = compact.get("wrapper_evidence")
                    if not (
                        policy[compact_index]
                        and isinstance(evidence, Mapping)
                        and evidence.get("event") == "context_compaction"
                        and evidence.get("author") == "policy"
                        and evidence.get("synthetic") is False
                    ):
                        continue
                    for read_index in range(compact_index + 1, len(episode)):
                        read = episode[read_index][0]
                        read_execution = cls._successful_execution(read)
                        if (
                            not policy[read_index]
                            or read_execution is None
                            or external_path not in str(read.get("action", ""))
                            or not str(read_execution.get("stdout", ""))
                        ):
                            continue
                        for reuse_index in range(read_index + 1, len(episode)):
                            reuse = episode[reuse_index][0]
                            reuse_execution = cls._successful_execution(reuse)
                            reuse_paths = (
                                reuse_execution.get("changed_paths", [])
                                if reuse_execution is not None
                                else []
                            )
                            if (
                                not policy[reuse_index]
                                or reuse_execution is None
                                or external_path
                                not in [str(path) for path in reuse_paths]
                            ):
                                continue
                            for execute_index in range(reuse_index + 1, len(episode)):
                                execute = episode[execute_index][0]
                                action = str(execute.get("action", ""))
                                if (
                                    policy[execute_index]
                                    and cls._successful_execution(execute) is not None
                                    and external_path in action
                                    and not action.lstrip().startswith(
                                        ("cat ", "head ", "tail ", "grep ", "sed ")
                                    )
                                ):
                                    return True
        return False
        ...
        for ordinal, path in enumerate(paths, start=1):
            ...
            for document in documents:
                ...
                if not isinstance(uid, str) or not uid:
                    ...
                    continue
                if uid in seen_uids and uid not in open_episodes:
                    self.errors.append(
                ...
                if not isinstance(item_id, str) or not item_id:
                    ...
                    continue
                current = open_episodes.setdefault(uid, [])
                if current and current[0][0].get("item_id") != item_id:
                    self.errors.append(
                        f"trajectory identity changed item_id within {uid!r}"
                    )
                if order != len(current):
                    self.errors.append(
                        f"trajectory {uid!r} action rows are not contiguous at order {order!r}"
                    )
                current.append((record, document))
                terminal = record.get("trajectory_terminal")
                done = record.get("rollout_done_flag")
                if terminal is True:
                    self.check(
                        done is True, f"trajectory {uid!r} terminal row is not done"
                    )
                    completed_episodes.append(current)
                    terminal_ids.append(item_id)
                    seen_uids.add(uid)
                    del open_episodes[uid]
                elif terminal is not False or done is not False:
                    self.errors.append(
                        f"trajectory {uid!r} nonterminal row has invalid terminal/done flags"
                    )
            for uid in sorted(open_episodes):
                self.errors.append(
                    f"trajectory {uid!r} has no terminal row in {path.name}"
                )
            self.check(
