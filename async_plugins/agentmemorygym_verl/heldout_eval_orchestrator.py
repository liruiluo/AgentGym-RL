...
def _verify_endpoint_source_counts(
    evaluation: HeldoutEvalPlan,
    endpoints: Sequence[HeldoutEndpointSpec],
) -> dict[str, int]:
    endpoint_counts = {endpoint.route_id: endpoint.task_count for endpoint in endpoints}
    expected_counts = evaluation.final_panel_authority.get("source_counts")
    if not isinstance(expected_counts, Mapping) or endpoint_counts != dict(
        expected_counts
    ):
        raise OrchestratorError(
            "held-out endpoint source-pool counts differ from the final panel "
            f"authority: endpoints={endpoint_counts!r}, sources={expected_counts!r}"
        )
...
def load_eval_orchestrator_plan(
    *,
    ...
    _verify_endpoint_source_counts(evaluation, endpoints)
...
def evaluator_cli_arguments(plan: HeldoutEvalPlan) -> list[str]:
    ...
    return [
        ...
        plan.model.source_commits["outer"],
        "--training-inner-commit",
        plan.model.source_commits["inner"],
        "--training-verl-commit",
        plan.model.source_commits["verl"],
