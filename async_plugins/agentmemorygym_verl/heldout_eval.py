...
class HeldoutEvalPlan:
    ...
    episode_count: int
    final_panel_authority: dict[str, Any]
...
def _schedule_counts_and_panel_authority(
    ...
    required_counts = {
        route_id: FINAL_PANEL_PER_ROUTE for route_id in CANONICAL_ROUTES
    }
    if route_counts != required_counts:
        raise ValueError(
            "formal held-out evaluation requires exactly 128 rows per route: "
            f"{route_counts!r}"
        )
    panel_authority = _mapping(
        schedule_metadata.get("final_panel_authority"),
        field="final held-out panel authority",
    )
    verified_panel = verify_final_panel_contract(
        panel_authority.get("manifest_path"),
        expected_manifest_sha256=panel_authority.get("manifest_sha256"),
        evaluation_contract_path=panel_authority.get("evaluation_contract_path"),
        expected_evaluation_contract_sha256=panel_authority.get(
            "evaluation_contract_sha256"
        ),
        expected_route_counts=route_counts,
    )
    if panel_authority != verified_panel:
        raise ValueError("final held-out panel authority summary drift")
    episode_count = sum(route_counts.values())
    if episode_count != FINAL_PANEL_TOTAL:
        raise ValueError(
            "formal held-out evaluation requires exactly 512 episodes: "
            f"{episode_count}"
        )
    if schedule_metadata.get("row_count") != episode_count:
        raise ValueError("held-out schedule manifest row count drift")
    return route_counts, episode_count, verified_panel
...
def load_eval_plan(
    *,
    ...
    route_counts, episode_count, panel_authority = (
        _schedule_counts_and_panel_authority(schedule_metadata)
    )
    ...
    return HeldoutEvalPlan(
        ...
        final_panel_authority=panel_authority,
...
def run_contract(plan: HeldoutEvalPlan, eval_config_sha256: str) -> dict[str, Any]:
    return {
        ...
        "schedule": {
            ...
            "final_panel_authority": plan.final_panel_authority,
        },
        "route_registry": {
