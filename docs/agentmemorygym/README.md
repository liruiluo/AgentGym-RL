...
- `evidence/20260702-scripted-search-baseline.md`：scripted SEARCH baseline / heuristic memory manager；full dev no-retry 为 `6/15`、retry5 semantic matcher 修复后为 `13/15`、soft-fallback verifier diagnostic 为 `15/15`，证明 SEARCH+verifier 接口有可解性但不是 RL/memory 提升结果。
