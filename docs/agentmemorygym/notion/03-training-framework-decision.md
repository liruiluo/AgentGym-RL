...
4. 基线 smoke：scripted SEARCH baseline / heuristic memory manager 已先跑通，strict no-retry 推进 dev、retry5 暴露 compatibility filter 残余失败、soft-fallback verifier diagnostic 跑到 `15/15`；后续再补 no-memory、full-context、fixed-RAG。
