...
- AgeMem ACL PDF / repo：确认其 memory tool taxonomy 为 LTM `ADD/UPDATE/DELETE` 与 STM `RETRIEVE/SUMMARY/FILTER`，与当前 AgentMemoryGym action 设计一致；本项目只借工具语义，不借其三阶段课程学习路线。
...
- `docs/agentmemorygym/evidence/20260701-latest-observation-rollout.md` 记录 latest-observation rollout context：`AGENTMEMORY_LATEST_OBSERVATION_PROMPT_SMOKE_OK`、`AGENTMEMORY_ROLLOUT_CONTEXT_ALIGNMENT_SMOKE_OK`。这证明训练数据路径不再把跨 session raw history 暴露给 actor/ref logprob 重算；latest-observation 仍允许当前 observation 内包含当前 session 自动 STM trace，但仍不等于完整模型 rollout 或 RL 训练结果。
- `docs/agentmemorygym/evidence/20260701-latest-observation-policy-smoke.md` 记录 scripted-policy rollout smoke：`AGENTMEMORY_LATEST_OBSERVATION_POLICY_SMOKE_OK`。它验证 latest-observation + memory tool contract 能闭合三条 bundled-shopping smoke 任务，并检查 `session_trace` 在 session 内出现、session 边界清空；但不是 LLM/vLLM rollout。
- `docs/agentmemorygym/evidence/20260702-session-stm-boundary.md` 记录本轮 session STM 边界修正：环境 observation 渲染 `Current session short-term history` 与 `Active retrieved/summary context` 两层，并给可见上下文加 `S*` / `C*` IDs；正式 RL 路径使用 policy-authored `SUMMARY{text, source_ids}` 和 `FILTER{keep_ids/drop_ids}`，不让环境后台调外部 LLM；`info["session_trace"]` 可用于行为分析，local compile/direct/policy smoke 已通过。
...
- 大文件存储与证据路径：MemoryArena product DB 已全量镜像到 `/media/cfs/ai-jingyan-train/luolirui.1/post-train/data/memoryarena-product-db/`，最终校验 `135 files / 13,517,161,526 bytes`，extra/missing/mismatch/part 均为 0；catalog-assisted conversion 证据在 `/media/cfs/ai-jingyan-train/luolirui.1/post-train/agentmemorygym-smoke-evidence/memoryarena_catalog_priority_convert_20260701-221648`。
- 正式 freeze 证据：`/media/cfs/ai-jingyan-train/luolirui.1/post-train/agentmemorygym-smoke-evidence/memoryarena_formal_freeze_20260701-234045`，marker 为 `AGENTMEMORY_MEMORYARENA_FORMAL_FREEZE_OK`，`tasks=150 / rows=900 / train/dev/test=120/15/15 / asin_catalog=900 / ambiguous=0 / catalog_paths=11`，source sha256 `4411a2da528a33dc6aca519b49cc225895363f18b2d19b191fddb501200134ef`。这仍是数据转换证据，不是 RL 训练或 memory 能力提升证据。
...
- Scripted SEARCH baseline 当前代码证据：`docs/agentmemorygym/evidence/20260702-scripted-search-baseline.md`，cpu9n no-retry full dev 目录 `scripted_search_baseline_dev_noretry_semanticfix5_20260702-074234`，marker `AGENTMEMORY_SCRIPTED_SEARCH_BASELINE_OK`，`episodes=15 / successes=6 / success_rate=0.4000 / mean_progress_score=0.5778 / search_calls=275 / rejected_buys=9`。
- No-memory diagnostic 当前代码证据：`scripted_search_baseline_dev_nomemory_semanticfix5_20260702-083000`，`policy=search-no-memory / episodes=15 / successes=0 / success_rate=0.0 / mean_progress_score=0.1889 / add_calls=0 / retrieve_calls=0`；failure audit 为 `{"attempt_budget_below_target_rank": 15}`。这说明 frozen dev 在该接口下确实依赖跨 session 信息。
- Full-context diagnostic 当前代码证据：`scripted_search_baseline_dev_fullcontext_semanticfix5_20260702-083000`，`policy=search-full-context / episodes=15 / successes=6 / success_rate=0.4 / mean_progress_score=0.5778 / add_calls=0 / retrieve_calls=0`；failure audit 为 `{"attempt_budget_below_target_rank": 7, "compatibility_filter_excluded_target": 2}`。这是绕过 memory-tool 决策的上下文诊断，不是 learned memory policy。
- SEARCH + verifier-feedback retry diagnostic 当前代码证据：`scripted_search_baseline_dev_retry5_semanticfix5_20260702-074234`，`episodes=15 / successes=13 / success_rate=0.8667 / mean_progress_score=0.9000 / search_calls=385 / buy_calls=91 / rejected_buys=10 / max_buy_attempts=5`。这是接口/可解性诊断，不是 one-shot policy 质量，也不是 RL/memory 能力提升。
- Failure audit 当前代码证据：`scripted_search_failure_audit_dev_retry5_semanticfix5_20260702-075320`，marker `AGENTMEMORY_SCRIPTED_SEARCH_FAILURE_AUDIT_OK`，残余 2 个失败为 `compatibility_filter_excluded_target`（`ck/cu`）。
- Soft-fallback verifier diagnostic 当前代码证据：`scripted_search_baseline_dev_softretry7_semanticfix5_20260702-075423`，`episodes=15 / successes=15 / success_rate=1.0 / mean_progress_score=1.0 / total_env_steps=674 / search_calls=420 / buy_calls=104 / rejected_buys=14 / compatibility_fallback=ranked-all-after-compatible`；failure audit `scripted_search_failure_audit_dev_softretry7_semanticfix5_20260702-075423` 显示该 run 无 failed steps。这证明接口可解，不是 RL/memory 能力提升。
- 9N 资源申请证据：`luolirui-1-amg-g5a-0702` 与 `luolirui-1-amg-g5b-0702` 已在 2026-07-02 08:46 CST 提交，资源请求均为 8×G5，当前 QUEUEING。devbox watcher 证据在 `/root/jd-coding/Post-train/docs/evidence/two-h8-20260702/`，tmux `watch_two_h8_0702`，cron `/root/bin/ensure_two_h8_watch_0702.sh`。占卡完成标准是平台 holder RUNNING + pod 内 auto-yield GPU guard 验证，不是 9N 排队行本身。
