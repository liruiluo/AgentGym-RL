...
- Important current markers: `MemoryArena / 电商捆绑购物优先`, `MemoryAgentBench 降级`, `先改本地文档和 Notion 文档，再改代码`, `代码草稿可以保留在工作树`, `中性 product_id`, `bundled_shopping_smoke.jsonl`, `AGENTMEMORY_DATA_VALIDATE_OK`, `SERVER_CLIENT_JSONL_SMOKE_OK`, `SERVER_CLIENT_SPLIT_SMOKE_OK`, `Stage 1a：0 卡本地代码/schema/server 检查`, `Stage 1b：真实单卡 GPU smoke`, `VERL_INIT_ENV_CLIENT_AGENTMEMORY_SINGLE_GPU_OK`, `AGENTMEMORY_ROLLOUT_CONTEXT_ALIGNMENT_SMOKE_OK`, `AGENTMEMORY_LATEST_OBSERVATION_POLICY_SMOKE_OK`.
