...
- **Objective:** evaluate the completed CompactionRL `global_step_400` actor on the frozen four-environment CAMG held-out panel and produce the four Table 2 success rates plus their equal-weight average.
...
- **Run scope:** the existing CompactionRL 8-card allocation only, after formal400 reaches its declared endpoint and the training owner has completed cleanup/holder handoff.
- **Non-goals:** no fourth 8-card request; no held-out or external evaluation before update400; no environment-specific rollout; no changes to training rewards, prompts, tasks, budgets, decoding, or graders; no interference with the other two 8-card owners.
...
- Policy checkpoint: the verified merged-HF publication derived from CompactionRL `global_step_400`.
...
1. Keep formal400 immutable and continuously supervised; seal registered training milestones and never run held-out data early.
...
3. Compose and byte-verify the 6,149-row schedule from the frozen held-out package; exercise only synthetic/CPU fixtures before update400.
4. After update400, verify trainer exit, checkpoint completeness, exact-source hashes, cleanup, and holder restoration; merge the actor to a content-addressed HF directory.
