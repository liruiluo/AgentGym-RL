...
- **Objective:** evaluate the completed CompactionRL `global_step_200` actor on the complete frozen four-environment CAMG held-out set and produce the four Table 2 success rates plus their equal-weight average.
...
- **Success artifact:** a resumable complete-set evaluation with one terminal receipt per assigned episode, exact source identities, complete failure accounting, a machine-readable metric summary, and a verified paper build containing the CompactRL row.
- **Verification:** focused evaluator tests, complete available `async_plugins/tests`, active-source audit proving the shared task-agnostic AgentLoop, exact schedule and checkpoint hashes, all scheduled terminal cells present, route-level denominator checks against the final complete split contract, paper checker/build, and pushed commits.
...
- Inputs: the final published complete-split package derived from the frozen leakage-unit assignments. The earlier `camg-native-eval-ready-v1-20260901` package is readiness evidence only and is not the final denominator.
- Counts: Shop 1,746; Coding `N_coding_full` from completed admission of all 10,381 raw held-out tasks across 36 repositories; DeepResearch 5,319; AutoResearch 169. The final total is `7,234 + N_coding_full` and must be read from the published complete split contract rather than hard-coded before Coding admission closes.
- Explicitly reject the obsolete 640-row Shop panel and 21-task one-per-repository Coding readiness panel as final Table 2 evaluation inputs.
...
3. Wait for the independent split owner to publish the complete split contract, then compose and byte-verify its full schedule; exercise only synthetic/CPU fixtures before update 200.
...
6. Require one terminal receipt for every row in the published complete schedule and exact route denominators; compute Table 2 metrics from finalized native evidence only.
